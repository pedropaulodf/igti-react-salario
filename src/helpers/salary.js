...
function calculateSalaryFrom(fullSalary) {
  ...
  const descontoPorceINSS = ((discountINSS / fullSalary) * 100 || 0).toFixed(2);
  const descontoPorceIRPF = ((discountIRPF / fullSalary) * 100 || 0).toFixed(2);
  const salarioLiquidoPorce = ((netSalary / fullSalary) * 100 || 0).toFixed(2);
